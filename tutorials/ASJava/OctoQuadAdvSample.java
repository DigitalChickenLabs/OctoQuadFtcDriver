...
public class OctoQuadAdvSample extends LinearOpMode
{
    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        // Switch second bank to pulse width id required;
        if (USE_TBE) {
        ...
        while (opModeIsActive())
        {
            ...
            for (int p = 0; p < 8; p++) {
                telemetry.addData("Pos", "%d %d (counts)", p, encoderDataBlock.positions[p]);
            }
            for (int v = 0; v < 8; v++) {
                telemetry.addData("Vel", "%d %d (counts/S)", v, encoderDataBlock.velocities[v] * 10);
            }
            telemetry.addData("Loop time", "%.1f uSec", avgTime.getMean()/1000);
            telemetry.update();
        }
    }
}
